...
function info(bot, message, data) {
    ...
    var addressRecord;
    var wbev, wbev2016, wbevRecords, wohnBevSum, rkSum, evSum, andereSum;
    try {
	...
	todesRecords = data.todesfalle(
            {'quarier_name': {"==": addressRecord.statistischesquartier}}
        ).get();
    ...
    wohnBevSum = wbev2016.sum("anzahl");
    answer(bot, message, "Wohnbevölkerung in der statistischen Zone _\"" + wbevRecords[0].stat_zone_name + "\"_ in 2016: *" + wohnBevSum + "*");

    // Konfession
    rkSum = wbev2016.filter({'kon_ogd_kurz': {'==': 'RK'}}).sum("anzahl");
    evSum = wbev2016.filter({'kon_ogd_kurz': {'==': 'EV-REF'}}).sum("anzahl");
    andereSum = wbev2016.filter({'kon_ogd_kurz': {'==': 'andere Konf.'}}).sum("anzahl");
    var chartData = [
        {
            value: rkSum,
            color: "#BE72D6",
    ...
    // Todesfälle
    var byYear = _.groupBy(todesRecords, 'jahr');
    var values = _.map(byYear, function(records) {
        var chSum = _.sumBy(records, 'anzahl');
        return {
            'chSum': chSum
        };
    });
    ...
    var lineFilepath = path.join(__dirname, '..', 'output', lineFilename);
    canvas.toBuffer(function (err, buf) {
        if (err) {
            throw err;
        }
        ...
        bot.api.files.upload({
            ...
            title: 'Todesfälle nach Herkunft und Geschlecht, seit 1971',
            filetype: 'png',
            channels: message.channel
        }, function(err, res) {
            if (err) {
